...
############################# ###############################################################
def plot_type_probability(aga_tp, abu_tp, OGE_tp,pomcp_tp, threshold, plotname):
    ...
    pomcp_tp = np.array(pomcp_tp)
    pomcp_error = pomcp_tp.mean(axis=0)  # .tolist()
    ...
    pomcp_error = np.array([1 - pomcp_error[t] for t in range(5, threshold)])
    #
   ...
   #     delta = (plot_aga_ci - aga_error)
        ...
        plt.fill_between(x, pomcp_error,  # - delta,
                         pomcp_error,  # + delta,
                         color='#CA8235', alpha=.15)
    # 3. Plotting
...
def plot_run_length_bar_1(aga_l,  abu_l,  OGE_l,pomcp_l,  plotname):
    # 1. Setting the figure
    global fig_count
    fig = plt.figure(fig_count, figsize=(6.4, 2.4))
    fig_count += 1
    ...
    pomcp_m = np.mean(np.array(pomcp_l))
    pomcp_s = info.calcConfInt(pomcp_l)
    ...
    #POMCP
    POMCP = axis.bar(4, height=pomcp_m, width=bar_w, yerr=pomcp_s - pomcp_m, color='#CA8235')

    # b. getting the current axis to label
    axis.set_ylabel('Number of Iterations')
    axis.set_xticks([1, 2, 3,4])
    axis.set_xticklabels(['AGA', 'ABU', 'OGE','POMCP'])
...
def plot_summarised(aga, aga_std, aga_ci,
    ...
    plot_pomcp = np.array([pomcp[t] for t in range(threshold)])
    ...
    plot_pomcp_ci = np.array([pomcp_ci[t] for t in range(threshold)])
    ...
    if show_confint:
        ...
        plt.fill_between(x, plot_OGE - delta,
                         plot_OGE + delta,
                         color='#F66095', alpha=.15)
        delta = (plot_pomcp_ci - plot_pomcp)
        plt.fill_between(x, plot_pomcp - delta,
                         plot_pomcp + delta,
                         color='#CA8235', alpha=.15)
...
for info in informations:
    ...
    general_pomcp = np.array([info.pomcp_levels[i] + info.pomcp_radius[i] \
                            + info.pomcp_angles[i] for i in range(info.threshold)]) / 3
    general_pomcp_std_dev = np.array([info.pomcp_levels_std_dev[i] + info.pomcp_radius_std_dev[i] \
                                    + info.pomcp_angles_std_dev[i] for i in range(info.threshold)]) / 3
    general_pomcp_ci = np.array([info.pomcp_levels_ci[i] + info.pomcp_radius_ci[i] \
                               + info.pomcp_angles_ci[i] for i in range(info.threshold)]) / 3
    ...
    plot_summarised(general_aga, general_aga_std_dev, general_aga_ci,
                    ...
                    general_pomcp, general_pomcp_std_dev, general_pomcp_ci,
...
for info in informations:
    ...
    pomcp_m.append(info.pomcp_mean_len_hist)
    pomcp_ci.append(info.pomcp_ci_len_hist)
    ...
    plot_run_length_bar_1(
                          ...
                          info.ABU_timeSteps,
                          info.OGE_timeSteps,
                          info.pomcp_timeSteps,
        info.name + '_Performance')
